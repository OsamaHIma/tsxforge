...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn("p-3 min-w-80", className)}
      classNames={{
        months:
        ...
        MonthCaption: ({ calendarMonth }) => {
          return (
            <div className="inline-flex gap-2">
              ...
              >
                <SelectTrigger className="min-w-30 gap-1 focus:bg-accent focus:text-accent-foreground">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {MONTHS.map((month) => (
              ...
              >
                <SelectTrigger className="min-w-10 gap-1 focus:bg-accent focus:text-accent-foreground">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {YEARS.map((year) => (
  ...
  (
    ...
    return (
      <Input
        ...
        name={name || picker}
        className={cn(
          "h-10 text-center text-base tabular-nums caret-transparent focus:bg-accent focus:text-accent-foreground [&::-webkit-inner-spin-button]:appearance-none",
          className,
        )}
        classNames={{
          wrapper: "w-[49px]",
...
const TimePicker = React.forwardRef<TimePickerRef, TimePickerProps>(
  ({ date, onChange, hourCycle = 12, granularity = "second" }, ref) => {
...
type DateTimePickerProps = {
  value?: Date;
  onChange?: (date: Date | undefined) => void;
  onChangeFormatted?: (formatted: string | undefined) => void;
  disabled?: boolean;
  ...
  (
    {
      ...
      hourCycle = 12,
      weekStartsOn = 1,
      displayFormat = { hour12: "dd-MM-yyyy" },
    ...
    const handleSelect = (newDay: Date | undefined) => {
      ...
      if (!value) {
        onChange?.(newDay);
        onChangeFormatted?.(format(newDay, outputFormat, { locale: loc }));
        setMonth(newDay);
        return;
      }
      ...
      onChangeFormatted?.(format(newDateFull, outputFormat, { locale: loc }));
    ...
    let loc = enUS;
    const { options, localize, formatLong } = resolvedLocale;
    if (options && localize && formatLong) {
            ...
            <div className="border-t border-border p-3">
              <TimePicker
                onChange={(date) => {
                  ...
                  onChangeFormatted?.(
                    date
                      ? format(date, outputFormat, { locale: loc })
                      : undefined,
                  );
                }}
                date={value}
