    ...
    return (
      <div className={cn("flex flex-col gap-2", className)}>
        ...
        <div
          ...
          className={`hover:bg-accent/50 data-[dragging=true]:bg-accent/50 has-[input:focus]:border-ring has-[input:focus]:ring-ring/50 flex min-h-46 flex-col items-center justify-center rounded-[15px] border-2 border-dashed border-main-600 p-4 transition-colors has-disabled:pointer-events-none has-disabled:opacity-50 has-[input:focus]:ring-[3px] ${
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center text-center">
            <UploadCloudIcon className="w-9 h-9 text-main-600 mb-2" />
            {file || hasExistingFile ? (
              <>
                <p className="mb-1.5 text-sm font-medium">File selected</p>
              ...
              <>
                <div className="flex items-center justify-center gap-1 text-sm">
                  ...
                  <span className="text-main-600 font-bold">browse</span>
                </div>
                <p className="text-xs text-muted-foreground mt-1">
                  {`Files up to ${formatBytes(maxSize)} are allowed`}
                </p>
              </>
            )}
          </div>
                  ...
                  <p className="truncate text-[13px] font-medium max-w-60">
                    {file
                      ? file.file instanceof File
                        ? file.file.name
                        : file.file.name
                      : initialUrl?.split("/").pop() || "Existing file"}
                  </p>
                  <p className="text-muted-foreground text-xs">
                    {file
                      ? formatBytes(
